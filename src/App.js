...
function App() {
  ...
  const enviarFormulario = () => {
    ...
    const url = idSeleccionado ? `http://localhost:8080/api/empleados/${idSeleccionado}` : "http://localhost:8080/api/empleados";
    fetch(url, {
  ...
  const eliminarEmpleado = (id) => {
    if (window.confirm("¿ELIMINAR REGISTRO DEL SISTEMA?")) {
      fetch(`http://localhost:8080/api/empleados/${id}`, { method: "DELETE" }).then(() => cargarEmpleados());
    }
  };
  ...
  const empleadosFiltrados = empleados.filter(emp => emp.nombre.toLowerCase().includes(busqueda.toLowerCase()));

  // --- LÓGICA DE ACCESO ---
  // Si el usuario NO está logueado, retornamos el componente Login y detenemos la ejecución aquí.
  if (!estaLogueado) {
    return <Login alLoguear={setEstaLogueado} />;
  }

  // Si está logueado, se muestra todo el Dashboard (Nexus OS)
  return (
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-8 space-y-6">
          ...
          <header className="flex justify-between items-center bg-white/5 backdrop-blur-md p-4 rounded-2xl border border-white/10">
            <div className="flex items-center gap-4">
              <span className="text-xs text-cyan-500/50 font-mono tracking-widest uppercase">System Status: <span className="text-green-400">Encrypted</span></span>
            </div>
            <div className="flex items-center gap-4">
            ...
            <section className="lg:col-span-1 space-y-6">
              <div className="bg-white/5 backdrop-blur-2xl p-6 rounded-3xl border border-white/10 shadow-2xl relative overflow-hidden group">
                ...
                <div className="space-y-4">
                  ...
                  {idSeleccionado && (
                    <button onClick={() => {setIdSeleccionado(null); setNombre(""); setCodigo("");}} className="w-full text-[9px] text-gray-500 hover:text-white uppercase">Cancelar</button>
                  )}
                </div>
              </div>
            </section>
            ...
            <section className="lg:col-span-2">
              <div className="bg-black/20 backdrop-blur-md rounded-3xl border border-white/5 overflow-hidden shadow-inner">
                <div className="p-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-center gap-4">
                  <h3 className="font-bold tracking-tight text-xl italic text-gray-300">Registros de <span className="text-cyan-400 underline decoration-cyan-500/30">Personal</span></h3>
                  <input
                      ...
                      {empleadosFiltrados.map((emp) => (
                        <tr key={emp.id} className="hover:bg-cyan-500/5 transition-all group">
                          ...
                          <td className="px-6 py-4 flex justify-center gap-4">
                            <button onClick={() => { setIdSeleccionado(emp.id); setNombre(emp.nombre); setCodigo(emp.codigo); }} className="text-blue-400 hover:text-blue-300 transition">✎</button>
                            <button onClick={() => eliminarEmpleado(emp.id)} className="text-red-400 hover:text-red-300 transition">✕</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
